...
def ingest(packet: dict):
    print("[INGEST CALLED]", packet["addr"], packet["ts"])
...
def start_app():
    print("[DEMO] starting injector")

    import threading
    t = threading.Thread(target=demo_injector)
    t.daemon = True
    t.start()
...
app = app  # IMPORTANT for gunicorn import
