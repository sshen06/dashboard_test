...
def ingest(packet):
    ...
    with state_lock:
        ...
        raw_log.appendleft(f"[{now_ts()}] NODE={addr} RSSI={packet.get('rssi',0)}")
    ...
    while True:
        for addr, s in state.items():
            ...
            packet = {
                ...
                "ax": math.sin(t*0.3+addr)*0.3,
                "ay": math.cos(t*0.3+addr)*0.3,
                "az": 1.0,
...
# =========================
# START THREAD (WORKS WITH GUNICORN)
# =========================
def start_demo():
    global demo_started
    if demo_started:
...
def api_state():
    with state_lock:
        ...
        for addr, n in nodes.items():
            node_list.append({
                **n,
                "age_s": round(now - n.get("last_seen", now), 1),
                "online": (now - n.get("last_seen", now)) < 60
            })

        # CRITICAL: never empty (prevents blank UI)
        if not node_list:
            node_list = [{
                "addr": i,
                "online": False,
